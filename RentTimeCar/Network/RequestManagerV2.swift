...
final class RequestManagerV2 {
    ...
    // MARK: - POST /api/contracts/create

    func createContract(with input: CreateContractInput) -> URLRequest? {
    ...
    // MARK: - POST /api/contracts/money-info

    func getContractMoneyInfo(clientIntegrationId: String, objectId: Int) -> URLRequest? {
        makeRequest(
            path: "/api/contracts/money-info",
            method: .post,
            body: ContractMoneyInfoBody(clientIntegrationId: clientIntegrationId, objectId: objectId)
        )
    }
